...
function setup_leavemessage($name, $email, $message, $groupid, $groupname, $info, $referrer, $canshowcaptcha,$phone='')
{
...
function create_thread($groupid, $username, $remoteHost, $referer, $lang, $userid, $userbrowser, $initialState, $link,$company,$phone,$email)
{
	global $mysqlprefix;
	$query = sprintf(
		"insert into ${mysqlprefix}chatthread (userName,userid,company,phone,email,ltoken,remote,referer,lrevision,locale,userAgent,dtmcreated,dtmmodified,istate" . ($groupid ? ",groupid" : "") . ") values " .
		"('%s','%s','%s','%s','%s',%s,'%s','%s',%s,'%s','%s',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,$initialState" . ($groupid ? ",$groupid" : "") . ")",
		mysql_real_escape_string($username, $link),
		mysql_real_escape_string($userid, $link),
		mysql_real_escape_string($company, $link),
