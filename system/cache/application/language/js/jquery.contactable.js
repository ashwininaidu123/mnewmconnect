...
(function(jQuery){
	...
	jQuery.fn.contactable = function(options) {
		...
		return this.each(function() {
			...
			*			</p>
			*			 <p>
			*				<label for="contactable-phone"> Phone <span class="contactable-green"> * </span></label><br />
			* 				<input id="contactable-phone" class="contactable-contact contactable-validate" name="phone" />
			*			</p>
			* 			<p>
			...
			*/
			...
			jQuery(this).html('<div id="contactable-inner"></div><form id="contactable-contactForm" method="" action=""><div id="contactable-loading"></div><div id="contactable-callback"></div><div class="contactable-holder"><p><label for="contactable-name">'+options.name+'<span class="contactable-green"> * </span></label><br /><input id="contactable-name" class="contactable-contact contactable-validate inputtext" name="name" /></p><p><label for="contactable-email">'+options.email+' <span class="contactable-green"> * </span></label><br /><input id="contactable-email" class="contactable-contact contactable-validate inputtext" name="email" /></p><p><label for="contactable-phone">'+options.phone+' </label><br /><input id="contactable-phone" class="contactable-contact inputtext" name="phone" /></p>'+dropdown+'<p><div id="subOptions"></div></p><p><label for="contactable-message">'+options.message+' <span class="contactable-green"> * </span></label><br /><textarea id="contactable-message" name="message" class="contactable-message contactable-validate" rows="4" cols="30" ></textarea></p><p><input class="contactable-submit" type="submit" value="'+options.submit+'"/></p><p class="contactable-disclaimer">'+options.disclaimer+'</p></div></form>');
			...
			jQuery('#contactable-dropdown').live('change',function(){
				var catgeory = jQuery(this).val();
				if(catgeory != 'Select'){
					$.ajax({
						type: "POST",
						url: "user/getSubcats/",
						data: "category="+catgeory, 
						dataType:"text",
						success: function(msg){
							jQuery('#subOptions').html(msg);
						}
					})
				}else{
					jQuery('#subOptions').html('');
				}
			...
			function submitForm() {
				...
				if(jQuery('#contactable-subcategory').attr('type') =='select-one')
					subcat = jQuery('#contactable-subcategory option:selected').text();
				else
					subcat = jQuery('input[name=subCategory]:'+jQuery('input[name=subCategory]').attr('type')+':checked').val();
				...
				jQuery.ajax({
					...
					data: {
						...
						category:jQuery('#contactable-dropdown').val(), 
						subcategory:subcat, 
