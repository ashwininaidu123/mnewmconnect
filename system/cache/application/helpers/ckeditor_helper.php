...
{
	// Initialization
	$return = cke_initialize($data['ckeditor']);
	
    // Creating a Ckeditor instance
    $return .= cke_create_instance($data['ckeditor']);
	

    // Adding styles values
    if(isset($data['ckeditor']['styles'])) {
    	
    	$return .= "<script type=\"text/javascript\">CKEDITOR.addStylesSet( 'my_styles_" . $data['ckeditor']['id'] . "', [";
   
    	
	    foreach($data['ckeditor']['styles'] as $k=>$v) {
	    	...
	    	if($k !== end(array_keys($data['ckeditor']['styles']))) {
				$return .= ',';
			}	    	
		...
		$return .= "CKEDITOR.instances['" . $data['ckeditor']['id'] . "'].config.stylesCombo_stylesSet = 'my_styles_" . $data['ckeditor']['id'] . "';
		</script>";		
    }   
