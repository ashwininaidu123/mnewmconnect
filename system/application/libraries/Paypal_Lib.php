...
class Paypal_Lib {
	...
	function Paypal_Lib()
	{
		...
		$this->paypal_url = $this->CI->config->item('pay_url');
		//'';
	...
	function image($file)
	{
		$this->submit_btn = '<input type="image" id="paynow" name="add" src="' . site_url('system/application/img/'.$this->button_path .'/'. $file) . '" border="0" />';
	}
...
*/
	...
	function paypal_form($form_name='paypal_form') 
	{
		...
		foreach ($this->fields as $name => $value){
			//echo $name."==>".$value."<br/>";
			//$arr=array('name'=>$name,'id'=>$name,'value'=>$value);
			//echo form_input($arr);
