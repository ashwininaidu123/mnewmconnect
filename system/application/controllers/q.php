...
class Q extends controller{
	...
	function r($id){
		...
		$qr_info=$this->r->getqrcodedetails($enc_id);
		$data['gid']=$qr_info->gid;
		$data['qrid']=$enc_id;
		$data['bid']=$qr_info->bid;
		$data['browser']=$this->getBrowser();
		$data['platform']=$this->getPlatform();
		
		$data['scanid'] = $this->r->addReport($data);
		
		if($qr_info->qruse=='2'){
			redirect($qr_info->webaddress);
		}
		
		
		$data['deal_info']=$this->r->getqrdealinfo($enc_id,$qr_info->bid);
		$fieldset = $this->configmodel->getFields('18',$qr_info->bid);
		...
		$data['qr_info'] = $qr_info;
		$this->load->view('qrlead',$data);
	}
	function savelead(){
		if($this->input->post('submit')){
			$res=$this->r->save_lead();
		...
		echo '<div>
				...
				<div class="info">';
					...
					echo "Thank You we Will Procees your Request";
		echo '</div>';
		}
	}
	...
	function postdeal(){
		if($this->input->post('submit')){
			...
			echo '<div>
					...
					<div class="info">';
			echo "You will receive Deal information soon....";
			//print_r($_POST);
			echo '</div>';
		}
	}
