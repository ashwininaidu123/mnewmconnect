...
//~ 
    //~ $(window).bind('enterBreakpoint768', function () {
        //~ $html.removeClass('sidebar-hidden').removeClass('sidebar-large').addClass('sidebar-medium');
...
//~ 
    $(window).bind('enterBreakpoint480', function () {
        $html.removeClass('sidebar-large').removeClass('sidebar-hidden').addClass('sidebar-thin');
        $('.sidebar-nav li.current').removeClass('active');
        $('.sidebar-nav li.current .submenu').removeClass('in');
        $sidebar.removeClass('collapse');
        sidebarHeight();
    ...
    $(document).on("click", "#menu-large", function () {
        $html.removeClass('sidebar-thin').removeClass('sidebar-hidden').addClass('sidebar-large');
        $sidebar_toggle.attr('id', 'menu-thin');
        $('.sidebar-nav li.current').addClass('active');
        $('.sidebar-nav li.current .submenu').addClass('in').height('auto');
...
jQuery(document).ready(function () {
	...
	$("#connectcall").submit(function(){
	 ...
	 $('#propertyname').live('change',function(event){
	  var r=$('#propertyname').val();
	   $.ajax({
			type: 'GET',
			url: "mconnect/getSitelist/"+r,  
			dataType: 'html',
			success: function(msg){
				$('#siteid option').each(function(i, option){ $(option).remove(); });
				$('#siteid').append(msg);
		   }
		 });
		});
...
$(document).ready(function() {
    ...
    $(add_button).click(function(e){ //on add input button click
        ...
        if(x < max_fields){ //max input box allowed
            ...
            $(wrapper).append('<div style = "padding-top: 10px;"><input class="form-control"  type="text" name="intrestopt[]" placeholder ="Your Option"><a href="#" class="remove_field">Remove</a></div>'); //add input box
        }
    });
...
$(document).ready(function() {
    ...
    $(add_button_loc).click(function(e){ //on add input button click
        ...
        if(x < max_fields){ //max input box allowed
            x++; //text box increment
            $(wrapper).append('<div style = "padding-top: 10px;"><input id="loc_image"  type="file" name="locimage[]"/><a href="#" class="remove_field">Remove</a></div>'); //add input box
        }
...
$(document).ready(function() {
    ...
    $(add_button).click(function(e){ //on add input button click
        ...
        if(x < max_fields){ //max input box allowed
            x++; //text box increment
            $(wrapper).append('<div style = "padding-top: 10px;"><input id="siteimage" type="file" name="siteimage[]"/><a href="#" class="remove_field">Remove</a></div>'); //add input box
        }
    });
...
var dis3 = document.getElementById("siteimage");
if(dis3){
$(document).ready(function () {
   if (this.value != "" || this.value.length > 0) {
      document.getElementById("siteimage").disabled = true;
      document.getElementById("add_image_button").style.display = 'none';
   }else{
	    document.getElementById("siteimg").disabled = false;
   }
