...
class DaemonProcess extends Daemon{
	var $basePath = '/var/www/mcube/src';
	var $pidFileLocation = '';
	var $logFileLocation = '';
	...
	function _doTask(){
		ob_flush();
		$running=true;
		$link = mysql_connect('localhost:/opt/lampp/var/mysql/mysql.sock', 'root', 'root');
		if (!$link) {
			...
			while($res=mysql_fetch_array($sql)){
				$file = $this->basePath."broadcast/".$res['filename'];
				if (($handle = fopen($file,"r")) !== FALSE) {
					...
					while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {$i++;
						...
						$network = mysql_fetch_object(mysql_query("SELECT network FROM mob_series WHERE series='".substr($data['0'],-10,4)."'"))->network;
					...
					fclose($handle);
					if(rename($file, $this->basePath.'broadcast/completed/'.$res['filename'])){
